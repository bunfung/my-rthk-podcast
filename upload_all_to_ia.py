#!/usr/bin/env python3
"""
上傳新集數到 Internet Archive
邏輯：
  1. 讀取 episodes.json（符合條件的集數）
  2. 跳過已在 ia_mapping.json 的集數
  3. 只上傳本地有 MP3 且未上傳過的集數
  4. 上傳成功後立即更新 ia_mapping.json
"""
import os
import json
import time
...
def main():
    ...
    episodes_path = f'{BASE_DIR}/episodes.json'
    try:
        with open(episodes_path, 'r', encoding='utf-8') as f:
            episodes = json.load(f)
        print(f"episodes.json: {len(episodes)} 集")
    except Exception as e:
        print(f"無法讀取 episodes.json: {e}")
        return
    ...
    # 整理需要上傳的集數（只上傳有 MP3 且未在 ia_mapping 的）
    to_upload = []
    for ep in episodes:
        ep_id = str(ep.get('id', ''))
        title = ep.get('title', '?')
    ...
    # 輸出統計供 shell 腳本讀取
    stats = {'success': success, 'failed': failed, 'total': len(ia_mapping)}
    with open('/tmp/ia_upload_stats.json', 'w') as f:
        json.dump(stats, f)
